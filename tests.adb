...
procedure Tests is
   Blank_Block : Block_8x8 := (others => (others => 0.0));
   Unit_Block  : Block_8x8 := (others => (others => 64.0));
   
   Identity_Q  : Quant_Matrix := (others => (others => 1));
   Zero_Q      : Quant_Matrix := (others => (others => 0));
   
   Test_Plane_4x4 : Image_Plane(1 .. 4, 1 .. 4) :=
     ((10, 20, 10, 20),
      (10, 20, 10, 20),
      (30, 40, 30, 40),
      (30, 40, 30, 40));
      
   Empty_Plane : Image_Plane(1 .. 0, 1 .. 0);
   Odd_Plane   : Image_Plane(1 .. 3, 1 .. 3) := (others => (others => 0));
begin
   ...
   declare
      Res : Block_8x8 := Forward_DCT(Unit_Block);
   begin
   ...
   declare
      DCT_Res : Block_8x8 := Forward_DCT(Unit_Block);
      IDCT_Res: Block_8x8 := Inverse_DCT(DCT_Res);
   begin
   ...
   declare
      Res : Integer_Block_8x8 := Quantize(Unit_Block, Identity_Q);
   begin
   ...
   declare
      Scale_Q : Quant_Matrix := (others => (others => 8));
      Res : Integer_Block_8x8 := Quantize(Unit_Block, Scale_Q);
   begin
   ...
   begin
      declare
         Res : Integer_Block_8x8 := Quantize(Unit_Block, Zero_Q);
      begin
   ...
   declare
      Int_B : Integer_Block_8x8 := (others => (others => 10));
      Scale_Q : Quant_Matrix := (others => (others => 5));
      Res : Block_8x8 := Dequantize(Int_B, Scale_Q);
   begin
   ...
   declare
      Res : Image_Plane := Subsample(Test_Plane_4x4, YUV_444);
   begin
   ...
   declare
      Res : Image_Plane := Subsample(Test_Plane_4x4, YUV_422);
   begin
   ...
   declare
      Res : Image_Plane := Subsample(Test_Plane_4x4, YUV_420);
   begin
   ...
   declare
      Res : Image_Plane := Subsample(Empty_Plane, YUV_420);
   begin
   ...
   begin
      declare
         Res : Image_Plane := Subsample(Odd_Plane, YUV_420);
      begin
   ...
   declare
      Int_B : Integer_Block_8x8 := (others => (others => 0));
      Res : RLE_Array := RLE_Encode(Int_B);
   begin
